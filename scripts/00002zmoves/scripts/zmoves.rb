...
# Power conversion table for Z-Moves
POWER_CONVERSION = {
  (0..55) => 100,
  (60..65) => 120,
  (70..75) => 140,
...
def calculate_z_move_power(base_power)
  # Use binary search for faster range lookup (assuming POWER_CONVERSION is sorted)
  low = 0
  high = POWER_CONVERSION_VALUES.length - 1
  while low <= high
    mid = (low + high) / 2
    return POWER_CONVERSION_VALUES[mid] if POWER_CONVERSION.keys[mid].include?(base_power)
    base_power < POWER_CONVERSION.keys[mid][0] ? high = mid - 1 : low = mid + 1
  end
  # If no match found, return default
  POWER_CONVERSION_VALUES.last
end

# Listen for key presses
Events.on(:key_press) do |event|
  # Check if pressed key is 'B' and skip unnecessary checks if not
  next unless event.key == :b

  # Get currently selected Pokemon (assuming method exists)
  selected_pokemon = $game_player.party[get_selected_pokemon_index]

  # Combine checks for item and Z-Move compatibility
  next unless selected_pokemon.item && Z_MOVES.key?(selected_pokemon.item.id)

  attempt_z_move(selected_pokemon, selected_pokemon.item)
end